...
-- query for:Updating an existing members address
UPDATE members
...
-- query for:Retrieving all books issued by specific employee-- OBJECTIVE:- Select  all books issued by the employee with emp_id='E101'
...
-- query for:Creating a new table with each book and total book_issued_count.
...
-- query for:Finding total rental income by category
...
-- query for:Listing the members who have registered in the last 180 days.
SELECT * FROM members WHERE reg_date >= CURRENT_DATE- INTERVAL 180 day ;


-- query for:Listing employees with their branch manager's name and their branch details
...
-- query for:Creating a table of books with rental price above a certain threshold 7usd
...
-- query for:generating a performance report for each branch  showing the number of books issued, the number of books
