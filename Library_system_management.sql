-- Library Management System Preject --
...
-- TASK 1.Create a new  book records --'978-1-60129-456-2','To Kill a Mockingbird','Classic',6.00,'yes','Harper Lee','J.B.Lippincott & Co.')"
...
-- TASK 4.Retrieve all books issued by specific employee-- OBJECTIVE:- Select  all books issued by the employee with emp_id='E101'
SELECT issued_book_name as book from issued_status where issued_emp_id='E101';


-- TASK 5. List members who have issued more than one book -- OBJECTIVE:-GROUP BY to find members who have issued more than one book.
...
SELECT category,SUM(b.rental_price)as  rental_income,COUNT(*) AS count FROM BOOKS AS  b join issued_status as ist
...
insert into members values('C120','Ashwin','345 vihar','2026-02-9'),('C122','Ritu','678 kumar','2026-01-12');
...
SELECT ist.*, rs.return_id FROM issued_status as ist LEFT JOIN return_status as rs ON ist.issued_id=rs.issued_id WHERE rs.return_id is NULL;


insert into issued_status values('IS151','C118','The catcher in the rye',current_date-interval 24 day,'978-0-553-29698-2','E108'),
...
-- TASK 13- Identify the members with overdue books-- Write a query to identify members who have overdue books (assume a 30 day return 
-- period).Display the member's_id,member's name ,book title,issue date , and days overdue.
...
-- TASK 14- Update booook status on return.Write a query to update the status of books in the books table to "yes" when they are returned
...
SET SQL_SAFE_UPDATES=0;
DELETE FROM RETURN_STATUS WHERE RETURN_ID ='RS125';
...
-- TASK 18. Identify members issuing High-risk books .Write a query to identify members who have issued books more than twice with the status
...
-- TASK 19.Stored procedure objectives:Create a stored procedure to manage the status of the books in the library system.Description :Write a stored 
-- procedure that updated the status of the library based on its issuance.The procedure should function as follows:The stored procedure should take 
-- the book_id as an input parameter.The procedure should first check if the book is available (status='yes').If the book is available it should be issued
-- and the status in the book table should be updated to 'no'.If the book is not available(status='no'),the procedure should return an error message
